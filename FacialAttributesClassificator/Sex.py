...
#  Classification Faces by Sex
#  Dataset CelabA pickle,   dictionary with keys [Images,Male]
...
datadf = pd.DataFrame(data, columns=['Male'])
datadf['Male'].value_counts()
...
plt.xticks(index+0.17, ('Mujer', 'Hombre'))
plt.legend()
...
cnn = define_model()
cnn.summary()
checkpoint = keras.callbacks.ModelCheckpoint("D:/best_model_Smile.hdf5", monitor='loss', verbose=1,save_best_only=True, mode='auto')
