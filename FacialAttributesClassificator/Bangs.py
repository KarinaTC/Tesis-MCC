...
#  Classification Faces with Bangs
#  Dataset CelabA pickle,   dictionary with keys [Images,Bangs]
...
datadf = pd.DataFrame(data, columns=['Bangs'])
...
plt.xticks(index+0.17, ('Sin flequillo', 'Con flequillo'))
plt.legend()
...
cnn = define_model()
cnn.summary()
checkpoint = keras.callbacks.ModelCheckpoint("D:/best_model_Bangs.hdf5", monitor='loss', verbose=1,save_best_only=True, mode='auto')
